...
import com.vbgps.dao.util.CollectionHelp;
import com.vbgps.dao.util.ReflectionUtility;
import com.vbgps.dao.util.StringUtil;
...
public class SqlBuild {
	...
	public static String buildInsertSql(Class<?> clazz, int type) {
		...
		if (StringUtils.isBlank(columsAndValues[0])) {
			...
			throw new RuntimeException("please config SqlField Annotation");
		}
	...
	private static String[] buildBatchInsertColumn(List<?> datas, Class<?> clazz, int type, String paramName) {
		...
		if (fields != null) {
			...
			columsAndValues[0] = StringUtil.substring(columns.toString());
			...
			columsAndValues[1] = StringUtil.substring(values.toString());
		}
	...
	private static String[] buildInsertColumn(Class<?> clazz, int type) {
		...
		String sqlColumns = StringUtil.substring(columns.toString());
		String sqlValues = StringUtil.substring(values.toString());
	...
	public static String buildUpdateColums(Object obj, int type, String tableAlias) {
		...
		if (fields != null) {
			for (Field field : fields) {
				...
				Object value = ReflectionUtility.getFieldValue(obj, field.getName());
	...
	public static String buildSelectCondition(Object obj, int type, String objectParamName) {
		...
		if (fields != null) {
			for (Field field : fields) {
				...
				case LIKE:
					builder.append(" ");
					builder.append(StringUtil.likeStr(field.getName()));
					break;
				case NOTEQUAL:
					builder.append(" != ");
					builder.append(buildAttrSql(field.getName(), objectParamName));
				...
				case NOTNULL:
					...
					break;
				case IN:
					Collection<?> collect = (Collection<?>) ReflectionUtility.getFieldValue(obj, field.getName());
					if (!CollectionHelp.isEmpty(collect)) {
						builder.append(" in (");
						if (CollectionHelp.isStringCollection(collect)) {
							builder.append(StringUtil.turnString(CollectionHelp.toListStr(collect)));
						} else if (CollectionHelp.isIntegerCollection(collect)) {
							builder.append(StringUtil.turnInt(CollectionHelp.toListInt(collect)));
						}
						builder.append(")");
					}
					break;
	...
	public static String buildSelectColumns(Class<?> clazz, int type, String alias) {
		...
		return StringUtil.substring(columns.toString());
	}
	...
	public static SqlTable existSqlTable(Class<?> clazz) {
		...
		if (sqlTable == null) {
			...
			throw new RuntimeException("please config SqlTable Annotation");
		}
		return sqlTable;
	}
	...
	public static Field primaryField(Class<?> clazz) {
		...
		if (primaryFeild == null) {
			...
			throw new RuntimeException("please config PrimaryKey Annotation");
		}
		return primaryFeild;
	}
}
